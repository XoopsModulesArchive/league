...
$query  = "select " . $xoopsDB->prefix("clubs") . " .nom, " . $xoopsDB->prefix("equipes") . " .id_club, " . $xoopsDB->prefix("equipes") . " .id_champ from " . $xoopsDB->prefix("equipes") . " , " . $xoopsDB->prefix("clubs") . "  where " . $xoopsDB->prefix("clubs") . " .id=" . $xoopsDB->prefix(
        "equipes"
    ) . " .id_club and " . $xoopsDB->prefix("equipes") . " .id='$equipe'";
...
$query  = "SELECT max(numero) from " . $xoopsDB->prefix("journees") . ", " . $xoopsDB->prefix("matchs") . " where " . $xoopsDB->prefix("journees") . ".id=id_journee and buts_dom is not NULL and id_champ='$champ'";
$result = $GLOBALS['xoopsDB']->queryF($query);
while (false !== ($row = $GLOBALS['xoopsDB']->fetchRow($result))) {
    $fin = $row[0];
}
...
$requete_sql      = "SELECT classement FROM " . $xoopsDB->prefix("clmnt_graph") . " WHERE id_equipe='$equipe' ORDER BY fin";
$resultat_requete = $GLOBALS['xoopsDB']->queryF($requete_sql);
...
while (false !== ($colonne = $GLOBALS['xoopsDB']->fetchBoth($resultat_requete))) {
    ...
    //if ($prec<$colonne[classement]){imageString($image, $titrePolice, $points[$i][0],($points[$i][1])-15 , $titre, $rouge);}
